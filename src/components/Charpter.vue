...
export default {
  ...
  methods: {
        ...
        this.$http.get(`/api/app/chapterInfo/${it.id}`).then(() => {
          this.$router.push({
            ...
            query: {
              ...
              system_course_id: this.$route.query.system_course_id
            }
          });
        });
        return;
      ...
      if (it.course_type == 5 || it.course_type == 8) {
        ...
        if (it.is_try_see || this.detail.is_buy || this.detail.is_join_study) {
          this.$http
            ...
            .then(res => {
              if (it.course_type == 3 && this.config.app_download_on == 1) {
                // 小班课 开启下载app功能
                this.$router.push({
                  name: "PlayMiddle",
                  query: {
    ...
    openFile(file) {
      if (this.$pub.checkLogin(true, "push")) {
        this.$http
          .get(`/api/app/chapterDatum/${file.datum_id}/${file.id}`, {
            system_course_id: this.$route.query.system_course_id
          })
          .then(res => {
            if (res.spell_status == 1) {
              window.location.href = res.file_url;
              // this.$pub.download(res.file_name, res.file_url);
            } else {
              this.$toast({
                message: "还未成团，暂无法下载"
              });
...
.charpter {
  ...
  section {
    ...
    .com-p {
      ...
      line-height: 56px;
      padding-left: 74px;
      background: url("../assets/empty.png") no-repeat 50px 20px;
